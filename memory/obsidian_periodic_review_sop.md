...
- `vault_profile.draft.md` 用户确认区是文件夹用途和长期主线的最高优先级上下文；SOP 是 Agent 自有工作经验。
...
- 新发现的文件夹用途和长期主线可以自动追加到 `vault_profile.draft.md` 的 Agent 候选区，但不能改用户确认区；复盘方法/SOP 不进入用户确认区。
